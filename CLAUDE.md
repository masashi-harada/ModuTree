...
├── ModuTree/
│   ├── Runtime/
│   │   ├── CSharp/                 # Pure C# — Unity非依存。C#環境はこのフォルダをコピー
...
│   │   │
│   │   └── PHP~/                   # PHP 8.1+ Runtime — PHP環境はこのフォルダをコピー
│   │       │                       # （末尾~でUnityインポート対象外・.meta生成なし）
│   │       ├── autoload.php        # require_once 1行でランタイム全体をロード
│   │       ├── Core/               # NodeState / BlackboardKey / Blackboard / BehaviourNodeData
│   │       ├── Engine/             # BehaviourTreeEngine（同期版）
│   │       ├── Nodes/              # 組み込みコンポジットノード + SubTreeNodeData
│   │       └── Serialization/      # BehaviourTreeSerializer（json_decode使用）
│   │
...
└── Samples/
    └── ModuTree/
        ...
        └── JankenSample/           # ワンショット実行サンプル（ジャンケンAI）
            ...
            └── PHP/                # PHP版サンプル（janken_api.php で動作確認可能）
                ├── Keys/           # JankenBBKeys.php（JankenHand int-backed enum を含む）
                ├── Nodes/          # IsPlayerChoseCondition.php / SelectHandAction.php
                └── janken_api.php  # APIエンドポイントのサンプル実装
```
...
| Assembly | 用途 |
|----------|------|
| `ModuTree.Runtime` | ランタイムコア（Unity 非依存、`noEngineReferences: true`）`Runtime/CSharp/` に配置 |
...
> **PHP Runtime** は `Runtime/PHP~/` に配置。フォルダ名末尾の `~` により Unity のインポート対象外となり `.meta` ファイルが生成されない。
...
`Runtime/PHP~/` は PHP 8.1+ 向けの同期実行ランタイム。Web API の1リクエストに対して AI を1回実行し結果を返す用途を想定している。

- AIデータの編集は Unity エディタで行い、出力された JSON をそのまま PHP 環境で使用する
- 非同期処理なし。`runToCompletion()` が完了まで同期的にループする
- PHP の `json_decode` を使用（MiniJson 不要）
- クラス名の自動解決: JSON の C# 型名末尾 `ClassName` → PHP クラス `ClassName` に自動マッピング

### PHP環境のセットアップ手順

```
my-server/
├── Runtime/          ← Runtime/PHP~/ の中身をコピー
│   ├── autoload.php
│   ├── Core/
│   ├── Engine/
│   ├── Nodes/
│   └── Serialization/
├── Keys/             ← プロジェクト固有のキー定義
├── Nodes/            ← プロジェクト固有のノード実装
└── AIData/           ← Unity で出力した JSON ファイル
```

### PHP版カスタムノードの書き方

C# クラス名と同じ PHP クラス名にすることで自動解決される（`BehaviourTreeSerializer::register()` 不要）。

```php
<?php
// Keys/MyBBKeys.php
class MyBBKeys {
    public static BlackboardKey $myValue;
}
MyBBKeys::$myValue = new BlackboardKey('myValue');

// Nodes/MyConditionNode.php
class MyConditionNode extends ConditionNodeData
{
    public int $threshold = 10;  // JSONの parametersJson から自動復元

    protected function onUpdate(): NodeState
    {
        $val = $this->blackboard->get(MyBBKeys::$myValue);
        return $val >= $this->threshold ? NodeState::Success : NodeState::Failure;
    }
}
```
...
C# の enum は整数値として JSON に保存される。PHP では int-backed enum で対応する。
...
$engine->blackboard->set(MyBBKeys::$myValue, $inputValue);

$jsonPath = 'AIData/MyTree.json';
$engine->initialize(file_get_contents($jsonPath), dirname($jsonPath));

$state = $engine->runToCompletion();  // Successまたは Failure になるまで同期実行
$result = $engine->blackboard->get(MyBBKeys::$resultKey);
```
...
| C# | PHP |
|----|-----|
| `async Task<NodeState> OnUpdateAsync(ct)` | `NodeState onUpdate()` |
| `Blackboard.Get(MyBBKeys.Foo)` | `$this->blackboard->get(MyBBKeys::$foo)` |
| `Blackboard.Set(MyBBKeys.Foo, val)` | `$this->blackboard->set(MyBBKeys::$foo, $val)` |
| `Engine.RunToCompletionAsync()` | `$engine->runToCompletion()` |
| `Engine.Reset()` | `$engine->reset()` |
| `BlackboardKey<T>` | `BlackboardKey`（型はPHPDoc） |
| `enum MyEnum { A = 0 }` | `enum MyEnum: int { case A = 0; }` |
